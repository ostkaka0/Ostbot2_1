...
namespace OstBot_2_1
{
    ...
    public class EEPlayer : SynchronizedSprite
    {
        ...
        public EEPlayer(int ID, string name, int frame, float xPos, float yPos, bool isGod, bool isMod, bool bla, int coins, bool purple, bool isFriend, int level)
            : base(null, 16)
        {
        ...
        public int overlaps(SynchronizedObject param1)
        {
            ...
            //int _loc_10 = 0;
            int _loc_11 = 0;
            if (param1.x < 16 || param1.y < 16 || param1.x >= Program.ostBot.room.width * 16 || param1.y >= Program.ostBot.room.height * 16)
            {
            ...
            for (int xx = -2; xx < 1; xx++)
            {
                ...
                {
                    if (_loc_3 + xx > 0 && _loc_3 + xx < Program.ostBot.room.width && _loc_4 + yy > 0 && _loc_4 + yy <= Program.ostBot.room.height)
                    {
                        for (int xTest = 0; xTest < 16; xTest++)
                        {
                            for (int yTest = 0; yTest < 16; yTest++)
                            {
                                if (hitTest((int)(xTest + _loc_2.x + xx * 16), (int)(yTest + _loc_2.y + yy * 16)))
                                {
                                    ...
                                    _loc_11 = Program.ostBot.room.getBotMapBlock(0, (int)(((xx * 16) + _loc_2.x + xTest) / 16), (int)(((yy * 16) + _loc_2.y + yTest) / 16)).blockId;
        ...
        void processPortals()
        {
            ...
            int _loc_10 = 0;
            double _loc_11 = 0;
            current = Program.ostBot.room.getBotMapBlock(0, cx, cy).blockId;
            ...
            if (!isgodmod && current == 242)
            {
                if (lastPortal.X == 0 && lastPortal.Y == 0)
                {
                    ...
                    Block currentBlock = Program.ostBot.room.getBotMapBlock(0, cx, cy);
                    ...
                    for (int x = 1; x < Program.ostBot.room.width; x++)
                    {
                        for (int y = 1; y < Program.ostBot.room.height; y++)
                        {
                            Block block = Program.ostBot.room.getBotMapBlock(0, x, y);
                    ...
                    while (loopIterator < targetPortalList.Count)
                    {
                        ...
                        _loc_4 = Program.ostBot.room.getBotMapBlock(0, lastPortal.X >> 4, lastPortal.Y >> 4).pt_rotation;
                        //Console.WriteLine("1: " + _loc_4);
                        //_loc_5 = world.getPortal(currentLoopPortal.x >> 4, currentLoopPortal.y >> 4).rotation;
                        _loc_5 = Program.ostBot.room.getBotMapBlock(0, currentLoopPortal.X >> 4, currentLoopPortal.Y >> 4).pt_rotation;
        ...
        override public void tick()
        {
            ...
            this.current = Program.ostBot.room.getBotMapBlock(0, cx, cy).blockId;
